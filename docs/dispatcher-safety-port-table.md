...
decision for the Notion → Linear dispatcher transition (BRO-266/#1303). "The dispatcher" here is
...
- **PORT — done**: the behaviour already lives in a *shared* module (`dispatch-guards.js`,
  `cmux-launch.js`, `dispatch-ledger.js`, `bsc-runner.js`, `bsc-next-model.js`) and `linear-next.js`
  already calls it. No work; the row documents that the port is complete and must not regress.
...
Verified against the tree at this branch's HEAD (files read: `bsc-next.js`, `cmux-launch.js`,
`dispatch-guards.js`, `dispatch-ledger.js`, `bsc-runner.js`, `linear-next.js`, plus grep of the
`push-*`, budget, recheck, and review-gate helpers).
...
|---|---|---|---|---|
| A1 | **Duplicate-dispatch guard** — refuse a 2nd workspace whose title matches a live (non-✅) session on this task | `scripts/lib/dispatch-guards.js : findLiveWorkspaceForTask` (+ `dispatch-ledger.js : titleMatchesSubject`) | **PORT — done** | Shared lib; `linear-next.js` calls it (both cmux + headless paths). Keep shared. |
...
| A4 | **Owner-close park guard** — a tab the owner closed without ✅ is "stop working this card"; refuse to reopen until `--force` | `dispatch-guards.js : parkedGuard` → `dispatch-ledger.js : parkedTasks` | **PORT — done** | Shared (takes a `cliName` param so the escape-hatch line prints the right script). `linear-next.js` calls it with `CLI_NAME`. |
| A5 | **Cross-session work-branch collision guard** — refuse if a local `worktree-<id>-*` / `job/<id>-*` branch already carries commits not on `origin/main` (another session already did the work) | `dispatch-guards.js : workBranchCollisionGuard` + `scripts/lib/worktree-branch-guard.js : listWorkBranchStatuses` | **PORT — TODO** | Guard is shared/pure but **only `bsc-next.js` calls it**. Port target: add the same `listWorkBranchStatuses` → `workBranchCollisionGuard` block to `linear-next.js` (its ledger taskId is `linear:BRO-N`, but the git branches are named by the *numeric* worktree/job id, so the id-anchored match needs the same id `bsc-next` passes — decide id mapping when porting). |
...
| A7 | **No dispatch-time auto-prune (design invariant)** — dispatch never closes ✅/idle tabs out from under the owner (owner rule 2026-07-15) | `bsc-next.js` main() comment block + absence of any close call; `linear-next.js` likewise never closes tabs | **PORT — done** | Invariant already holds on the Linear path (it only ever *appends* dead breadcrumbs, never mutates cmux). Documented so a future edit doesn't reintroduce a dispatch-time close. |

## B. Verifiability / acceptance gates (card-text-shaped)

| # | Safety behaviour | Lives today (file : function) | Port or Delete | Rationale / target |
|---|---|---|---|---|
| B1 | **Verify-command gate** — refuse to dispatch a card with no runnable `## Acceptance criteria` command (unless `--allow-unverifiable` or `VERIFY: owner-judgment`); arms the nightly recheck | `scripts/lib/verify-gate.js : evaluateVerifiability` (re-exported via `dispatch-guards.js`) | **PORT — done** | Shared; `linear-next.js` calls it on `issue.description`. Note: Linear has **no truncated-mirror degraded path** — it always has the full description, so it refuses outright instead of warning. |
| B2 | **Headless human-gate refusal** — refuse `--headless` when an unattended session can't *finish* the card (visual-qa approval, owner decision, long external wait) | `scripts/lib/headless-dispatchability.js : classifyHeadlessDispatchability` (re-exported via `dispatch-guards.js`) | **PORT — done** | Shared; `linear-next.js` runs it for `routing.mode === 'headless'`. |
...
| B4 | **Terminal-state guard (Linear analog of B3 + completed-launch)** — refuse dispatch of a Done/Canceled Linear issue | `scripts/lib/linear-dispatch.js : checkTerminalStateGuard` (called in `linear-next.js`) | **PORT — done** | Already the Linear-native replacement for B3 + `completedLaunchGuard`. Keep. |
| B5 | **Completed-launch guard** — refuse launching a task already `status:completed` (typo'd task #) unless `--force`; RECHECK-AFTER-exempt | `bsc-next.js : completedLaunchGuard` (+ `recheck-stamp.js : parseRecheckAfter`) | **DELETE** | `task.status` is a Notion-mirror/native field. Superseded by **B4** on the Linear side. Delete with the Notion path. |

## C. Launch-verification primitive (`cmux-launch.js`) — shared

| # | Safety behaviour | Lives today (file : function) | Port or Delete | Rationale / target |
|---|---|---|---|---|
| C1 | **Launch verified, not assumed** — state machine confirms the typed command actually started claude before reporting success; slow-boot cap (`slowBootCapSec=360`) never triggers a destructive relaunch | `cmux-launch.js : launchCmuxSession` / `waitForLaunchOutcome` + `cmux-launch-state.js : decideLaunchWait` | **PORT — done** | Shared primitive; `linear-next.js` calls `launchCmuxSession` with the same `verifyTimeoutSec:90 / slowBootCapSec:360 / lateAdoptSec:60` budget. |
...
| C4 | **Auth preflight** — refuse to open a doomed tab when claude auth is broken (fail-closed; kill switch `CMUX_AUTH_PREFLIGHT_DISABLED`) | `cmux-launch.js : shouldRefuseForAuth` + `claude-cli.js : preflightAuth` | **PORT — done** | Inside the shared primitive. |
...
| D3 | **Succession-cap owner page** — best-effort digest alert when the cap is hit | `bsc-next.js : pageSuccessionCapExceeded` + `owner-alert-router.js : routeAlert` | **PORT — TODO** | Ports together with D1. |
...
| E3 | **CI-red claim auto-invocation** — record a CI-red fix claim at dispatch so another session's push-gate sees the symbol is being worked | `bsc-next.js : recordCiRedClaim` + `ci-red-dispatch-heuristic.js : extractCiRedTarget` + `ci-red-claims.js : appendClaim` | **PORT — TODO** (low priority) | Cross-session safety that is genuinely dispatcher-agnostic, but `extractCiRedTarget` reads the Notion task+card shape. Port target: feed it the Linear issue title/description, invoke from `linear-next.js` at its confirmed-dispatch points. Niche; safe to defer. |
...
| F2 | **Model floor** — dispatched sessions never inherit the interactive default (Fable); floor is Sonnet, Opus only on hint/triage (stops "9 Fable workspaces in one night") | `scripts/lib/bsc-next-model.js : resolveModel` | **PORT — done** | Shared; `linear-next.js` calls `resolveModel`. |
...
| G2 | **Per-task headless lease** — one live headless job per task (cross-dispatcher; atomic file lease, PID-checked) | `scripts/lib/bsc-runner.js : acquireLease` / `releaseLease` / `pidLooksLikeClaude` | **PORT — done** | Shared; `linear-next.js` reaches it via `runJob`. Note the lease is blind to a live cmux **tab** — which is why both dispatchers *also* keep the `findLiveWorkspaceForTask` tab check on the headless path. |
...
1. **H1 — Linear acceptance recheck.** `linear-next.js` already arms the verify gate and writes
   `verifyCmd`+`linearId` to the ledger, but nothing re-runs it for Linear issues. Without this the
   whole verify-gate contract is half-wired on the Linear side.
2. **A5 — work-branch collision guard** and **C6 — launcher-outage detector.** Both are shared/pure
   already; the only gap is that `linear-next.js` doesn't *call* them. Cheap, high-value.
