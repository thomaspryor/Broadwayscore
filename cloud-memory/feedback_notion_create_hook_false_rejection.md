...
description: notion-create-verify.sh greps stdout for "REJECTED" case-insensitively to detect validation failure; card notes containing the English word "rejected" falsely trigger the FAILED breadcrumb, blocking subsequent Bash calls
type: feedback
...
3. Long-term fix for the hook: replace `grep -qi "REJECTED"` with `grep -q "❌ REJECTED ("` or check for `__NOTION_CARD_ID__=` absence as the failure signal instead of string scanning.
