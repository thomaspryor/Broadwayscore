...
**Fixed 2026-08-18:** renamed `the-sound-of-music-2026`→`the-sound-of-music-2027` (id now matches its real openingDate year, so discovery's id-collision guard skips it instead of re-minting a duplicate). Systemic fix: `extractCandidateTokens` (scripts/lib/validation-setdiff.js) now also matches bare, unquoted show-id-shaped tokens in error prose — same-title-sibling errors interpolate the OTHER show's id as free text with no quotes/parens, which is exactly what produced the empty attribution set here. Separately found + fixed: `check-cron-health.yml`'s chronic-stale escalation step (disposition:'auto', the mechanism that's supposed to auto-file a tracker after 3+ consecutive stale days) was missing `LINEAR_API_KEY` in its env — it correctly detected "Update Show Status" as chronically stale every day for a week but its Linear issue-creation call always failed silently, so no one was ever paged. That's why this needed a human-triggered card instead of the auto-escalation firing on its own.
