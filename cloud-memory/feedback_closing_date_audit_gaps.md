...
**UPDATE 2026-07-14 — 5th gap found and closed: `title_mismatch` = removed page was still silent.** broadway.com REMOVES a show's page after it closes, so the 2026-06-21 fix never fired for exactly the shows most likely to be stale-open: the removed page fails `pageMatchesShow()` → `errors: broadway_com_title_mismatch`, not the empty-calendar branch. Celebrity Autobiography closed 2026-06-21, page removed, sat stored open-through-9/6 for 3+ weeks of green daily runs. Shipped (commit cdf4517dd26): `scripts/lib/closing-audit-classify.js` classifier routes BOTH kinds to POSSIBLY_CLOSED; possibly-closed entries now enter the triple-signal auto-fix (agreement = press cluster earlier than stored; for removed pages the press date must also be in the past to avoid slug-collision auto-retractions of running shows); flood cap of 5 title-mismatch flags/run guards against pageMatchesShow() breakage. E2E-verified: Celebrity fixture auto-fixed 9/6→6/21 from 5 press sources. Same-day sweep caught a **6th gap**: The Balusters (closed 6/21, limited run, NO stored closingDate) — possibly-closed logic required a stored close, so no-close shows with empty title-confirmed calendars dropped to errors. Also fixed (commit a8218150901): empty_schedule + no stored close → POSSIBLY_CLOSED; auto-apply needs past press date. Remaining known gaps: OB/WE (unchanged), short-title shows skip pageMatchesShow entirely (Job/SIX class — pre-existing), title_mismatch + no stored close still errors (slug-collision-heavy class, by design).
