...
**Exception — some workflows hardcode `ref: main` on their OWN checkout step**, deliberately, so a scheduled safety-net job never runs untrusted branch code even when manually dispatched (e.g. `.github/workflows/audit-imageless-scored-shows.yml`'s "Checkout repository" step has `with: {ref: main}` regardless of the dispatch ref). `--ref <branch>` only selects which version of the *workflow YAML* runs — it does NOT override an internal step that pins its own checkout to `main`. Wasted two full ~10-12min dispatches (2026-08-23, card #1456 self-heal investigation) believing a branch-dispatch was testing a not-yet-merged fix to `scripts/lib/core-data-merge-registry.js`, when the checkout step silently pulled `main`'s stale copy the whole time. **Before relying on a branch-dispatch to validate a fix: grep the target workflow's own checkout step(s) for a hardcoded `ref:` first** — if present, the only way to live-test is merge-then-dispatch, same as this file's advice would otherwise contradict.
