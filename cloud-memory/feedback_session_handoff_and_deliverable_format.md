...
**Correction 2026-07-21 (final):** a venue-fix session concluded "Claude Design unusable, artifact-only" — WRONG: it tested only direct claude.ai/project/<id> URLs (which no-permission for the owner) while the owner confirmed the claude.ai/design → Design systems pane opens "iOS App — Proposed Designs" fine. Venue = that project via the design PANE path; never send the owner raw project URLs. App CLAUDE.md §Design Proposals is authoritative.

## One asker per decision (owner escalation 2026-08-03)
When crowning a successor while an owner decision is pending, exactly ONE place may ask: either resolve the decision in the current chat BEFORE crowning, or delegate the ask to the successor — never both. The 2026-08-03 iOS-feedback handoff asked in the crowner's final message AND told the successor to ask; the owner had to answer twice and escalated ("Why do I have to answer in two places?").
**How to apply:** put the decision + options INTO the handoff brief with a clear default; the brief is the single source of truth. When the owner answers anywhere, immediately stamp `DECIDED <date>: <choice> — do NOT ask again` into the brief so every reader sees it resolved.
