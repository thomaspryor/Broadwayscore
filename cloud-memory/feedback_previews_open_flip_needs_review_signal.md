...
**2026-06-25 extension — open-signal decoupled from score-threshold (`openSignalFromReviews`):** the count-based Check 2d above could NEVER fire for a genuinely-open but thinly-reviewed show — e.g. Label•less (2 OB reviews, will never reach the 3-review gate) sat in `previews` forever. Two compounding causes: (1) `gather-reviews.js` SKIPS previews shows, so they can't accumulate reviews to reach the threshold (deadlock); (2) "open" (a status fact) was conflated with "scoreable" (>= threshold). Fix: `openSignalFromReviews(show, entry, isDateReached)` in `opening-signal.js` — a previews/upcoming show with `>=1` clean review dated on/after `previewsStartDate` has demonstrably opened (critics review at/after press night; reviews.json is already wrongProduction/wrongShow/roundup-free). Check 2d now flips on EITHER the score-threshold OR this open-signal. The site's score gate is UNCHANGED — the show reads "Now Playing, reviews pending" until the threshold, the honest state. Empirically: after flipping, white-rabbit (4→7 reviews, cs 76) and label•less (2→3, cs 46) crossed their gates because gather stopped skipping them once open. Guards: press-night must be reached (no future dates → no Check-2c oscillation) and >= previewsStartDate (no stale-leak). `findStuckPreviews(shows, countMap, isDateReached)` now surfaces both signal types (tagged `signal`). NOTE: triage before blind-flipping a stuck-previews list — some are not-yet-open (future openingDate), some non-reviewable (art installations, standup, immersive experiences).
