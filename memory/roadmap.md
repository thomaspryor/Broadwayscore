# Roadmap — Last updated 2026-04-11 (undici/axios vuln cleanup + DiscountTicketsNav OWE)
...
- **34 vercel CLI devDep vulns (remaining after 2026-04-11 cleanup)** — All inside `vercel@50.43.0` → `@vercel/node` / `@vercel/backends` / `@vercel/fun` / `path-to-regexp` / `next` subtree. devDep only, not shipped, but 24 are high severity. Options: (a) wait for upstream vercel fixes; (b) drop `vercel` devDep entirely and let CI `npx vercel` download on each run (slower, cleaner); (c) aggressive `overrides` to force newer undici into `@vercel/node` (risk: API break 5.x→6.x+). `npm audit --audit-level=critical` still passes — LOW priority maintenance cleanup. See `memory/roadmap.md` undici cleanup entry for the full dep tree.
- **FooterEmailCapture.tsx OWE routing** — Still in the `regression-guards` co-occurrence exclude list (comment: "OWE routed via hook"). Unlike DiscountTicketsNav it uses a separate runtime mechanism (`useFormspreeCapture`) to route off-west-end submissions. Worth auditing whether the type-level widen pattern (`DiscountMarket` now includes `'off-west-end'`) could replace the hook-based workaround, removing another exclude-list entry. LOW priority — runtime behavior is correct; this is code-clarity cleanup.
- **Test Suite rate-limit contention on `Checkout review-texts`** — GitHub REST API rate limit can be burned by parallel Claude sessions + the `actions/checkout` default-branch lookup for private repos. When it hits, E2E Tests + Data Validation fail at the checkout step, not during tests. Retry usually passes. Would be nice to pin the default branch in the checkout action call (avoids the API lookup) — see `.github/actions/checkout-review-texts`. LOW priority — retry is cheap.
- **`wet-title-extraction.test.mjs:219` HIGH confidence assertion** — 1 pre-existing local unit-test failure (`null !== 'high'`); some WET title pattern is returning null confidence when the test expects high. Didn't block CI in the 2026-04-11 session (test is in `no-data-dependency` scope but CI skips that step differently from `npm run test:unit` locally). Find the fixture title returning null + compare to extractor. LOW urgency.

**Gold list / OUTLET_TIERS follow-ups (from 2026-04-11 session):**
- **LLM prompt tier label drift (cosmetic, do not rescore)** — `scripts/llm-scoring/input-builder.ts:141-142` injects the outlet tier as a human-readable string (`"(Tier 2 (notable outlet))"`) into the LLM's user message context. 151 reviews from thestage/timeout-london/financialtimes/daily-mail/artsdesk were scored with the wrong tier label before the April 10 outlet-tiers.js fix. BUT: the system prompt never instructs the model to weight by tier — the rubric is sentiment/star-based. The outlet NAME (a stronger signal than the tier parenthetical) is also in the prompt. Effective impact on final scores: near-zero per review, likely nothing crosses a bucket boundary. Rescoring would cost $1-3 + 20-40 min runtime for no measurable user benefit. Decision: leave alone. The composite-tier weighting (the REAL tier math) was fixed at the engine level so all new shows are fine. Roadmap only because the audit trail should exist if a future incident points here.
- **OUTLET_TIERS write-side guard** — `src/config/outlet-tiers.json` is read-authoritative (both engine.ts and compute-critic-score.js load it). But `outlet-registry.json` is auto-updated by scraping scripts and can silently clobber tier values for the 81 outlets listed in outlet-tiers.json. Root-cause fix: teach registry-update scripts to check outlet-tiers.json and skip the tier field for listed outlets. Prevents future drift of the class that caused the April 10 Stereophonic incident. MEDIUM priority.
- **Editorial gold-list dashboard** — A single page showing per-show gold list memberships ("Stereophonic — on Critical Gold 2023-2024 rank 1, Audience Gold 2023-2024 rank 3"). Useful for social posts, press kits, year-end roundups. Data already exists in `gold-lists-computed.json::memberships`. Reuses `GoldListBadge`. LOW priority — cosmetic editorial tool.
- **West End audience/box-office/hot-ticket gold lists** — 4-market symmetry for non-critic lists. Audience Gold WE probably has data (WET, theatre.reviews, Stagedoor, SeatPlan). Box Office + Hot Ticket are Broadway-only data sources. Scope: add Audience Gold WE first; defer Box Office/Hot Ticket until WE data sources exist. MEDIUM priority — completes the market symmetry story.
- **Per-market OG images for /lists pages** — All 7 gold list pages currently share `/og/home.png` for social previews. A tweet of the Off-Broadway list and a tweet of the Broadway list are indistinguishable on preview. Per-list (or at least per-market) OG images would noticeably lift social CTR. Needs OG generation pipeline work. LOW priority — share velocity not proven yet.

## Recently Completed

### undici/axios critical vuln cleanup + DiscountTicketsNav OWE (2026-04-11)
- **Symptom** — `Dependency Audit` job in Test Suite was failing; handoff from prior session claimed the blocker was `undici` GHSA-phc3-fgpg-7m6h (DeduplicationHandler DoS, marked critical in GHSA), pointing at `npm audit fix --force` as the remedy.
- **Actual blocker** — `axios` SSRF (`NO_PROXY` Hostname Normalization Bypass + Unrestricted Cloud Metadata Exfiltration via Header Injection Chain, both `<1.15.0`). The undici advisory had been reclassified to moderate by npm audit, so `--audit-level=critical` was flagging only axios. Handoff was partially outdated.
- **Fix (commit `8018951475`, deploy run `24273950262` success)**
  - `axios` 1.14.0 → 1.15.0 (fixes the 1 critical)
  - `vercel` 50.39.0 → 50.43.0 (latest)
  - `overrides.cheerio.undici` → `^6.24.1` (dedupes cheerio's vulnerable undici 7.20.0 to the safe 6.24.1 already present elsewhere in the tree)
  - `npm audit fix` (non-breaking) — cleaned up tar, flatted, basic-ftp, brace-expansion, picomatch high/moderate
  - `--force` was NOT used (would have downgraded vercel from 50.x back to 32.3.0 — the handoff misread the direction).
- **Result** — `npm audit --audit-level=critical` exits 0. 39 → 34 total vulns (1 low, 12 moderate, 21 high, **0 critical**). The remaining 34 are all inside the vercel CLI's own dep tree (@vercel/node 5.28.4 undici, @vercel/backends, path-to-regexp, next, etc.) — devDep tooling only, not shipped, would need upstream vercel fixes.
- **DiscountTicketsNav.tsx OWE handling (commit `1e829cd8e4`)** — The follow-up Test Suite run surfaced `regression-guards.test.mjs` co-occurrence guard flagging `src/components/DiscountTicketsNav.tsx` for having `=== 'west-end'` without `off-west-end`. Widened `DiscountMarket` union to include `'off-west-end'` (routes to same `/west-end/*` prefix — OWE shows share West End discount routes, no separate OWE hub exists). A parallel session had added the file to the exclude list at `6c88633d5a` 8 min earlier; that entry was then removed as redundant in `0f1450496c` — the type-level fix makes the guard actively protect this file going forward instead of silently excluding it.
- **tests/unit/data-core.test.ts:172 downlevel-iteration fix** — The market-partition test fix in `9b7df433a2` introduced `[...allIds]` (Set spread) which needed `--downlevelIteration` under the default ES3 target, breaking `tsc --noEmit` and `next build`. Swapped to `Array.from(allIds)` — same semantics, no tsconfig change.
- **Live verification** — Test Suite run `24290179995` (retry after clearing GitHub REST API rate limit contention on `Checkout review-texts`): **all 7 jobs green**, including Dependency Audit. Vercel deploy `24287931604` live on broadwayscorecard.com; Playwright golden-path confirmed desktop + mobile homepage, wicked show page, score badges, 0 console errors.
- **Notion card** — `P1: undici critical DoS vulnerability — GHSA-phc3-fgpg-7m6h` (`33f637c5-416f-8136-977d-c788388ede9b`) marked Done with completed-date 2026-04-10.

### LLM Refusal Synopsis Guard (2026-04-11)
- **Symptom** — Graham100: The Centennial Celebration (off-broadway-2026) and 21 other shows were rendering Claude refusal paragraphs ("I do not have enough information about the specific plot or premise of X to provide a factual synopsis...") as their synopsis on the live show page AND leaking into the SEO meta description via `src/app/show/[slug]/page.tsx:126`. User screenshot flagged it.
- **Root cause** — `isValidSynopsis()` in `scripts/auto-fix-show-data.js` only checked length, accessibility keywords, marketing openers, and trailing punctuation. A 300-char grammatical LLM refusal passed every gate and was written to `shows.json`. No deploy-time guard caught it either.
- **Fix (commit `2ba4f869ab`, deploy `24286027180` success)**
  - New shared helper `scripts/lib/synopsis-validation.js` — 17 refusal regex patterns + `isValidSynopsis()` + `isLlmRefusal()` + `detectRefusalPattern()`. Pure function, 7/7 unit tests pass (Graham100 phrasing, "I'm afraid", "Unfortunately, I", "as an AI", real synopses, accessibility, too-short).
  - `scripts/auto-fix-show-data.js` now imports the shared `isValidSynopsis` so future Claude refusals are rejected at generation time (line 342 gate).
  - `scripts/pre-deploy-check.js` self-heals any refusal still in `shows.json` before every Vercel build — same pattern as orphan-image and venue-category auto-fixes. Writes `synopsis: null`, logs show ID + matching pattern.
  - `data/shows.json` — 22 existing refusals cleared (Graham100, La Traviata, Tristan und Isolde, Mercury, Clara, Gotta Dance!, Inter Alia, YOAH, Entangled, LOS SOLES TRUNCOS, Age Is a Feeling, Here There Are Blueberries, Jamire Williams, Spring Dances, R.O.I., Lauder, Milk and Honey, Can I Be Frank?, Youth in Flames, Catch of the Day, David Copperfield, THIS IS NOT ABOUT ME).
- **Live verification** — `curl` on Graham100 post-deploy: 0 matches for "I do not have enough information" (was 2 — body + meta description). Spot-checked 4 more shows: all 0. Meta description now `"Read critic reviews for Graham100: The Centennial Celebration Off-Broadway..."` instead of the refusal.
- **UI behavior with null synopsis** — `src/app/show/[slug]/page.tsx:719,1158` truthy-check `show.synopsis`, so null cleanly hides the section. The next scheduled `auto-fix-show-data.js` run will re-attempt TodayTix/Wikipedia enrichment; refusal guard active so any new Claude hedge stays blocked.
- **Related pre-existing tech debt noticed (not fixed — out of scope):** 8 shows have truncated-mid-word synopses (fetch-pipeline cut them off), 782 shows have empty-string synopsis (hidden OK), 10 WE review files from commit `7c944b22d1` have aggregator scores in `originalScore` instead of `aggregatorStars`. (TS error in `tests/unit/data-core.test.ts:172` `Set<string>` iteration → FIXED in the 2026-04-11 undici cleanup session, commit `1e829cd8e4`.)

### Red Test Suite fix — commercial sources + cats Observer + aggregator scores (2026-04-10)
- **6 validator errors cleared** — Test Suite was red on `main` for 15+ consecutive runs. Run `24272002372` went green after this session's work landed.
- **Parallel-session collision** — 4 systematic-guard commits (`e667972132`, `da947e74`, `616e22dad2`, `d46463cee1`) were produced independently by another Claude session with matching reasoning while this session was implementing the same fixes. Recorded as feedback memory (`feedback_check_parallel_sessions.md`): always `git log --since="2 hours ago"` before implementing non-trivial infra fixes.
- **Data fixes (unique to this session, pushed via `gh api`)** — The script fixes alone couldn't repair existing contaminated data. Cleared via private-repo API edits:
  - Deleted `cats-the-jellicle-ball-2026/observer--unknown.json` duplicate (DTLI misattributed a `theguardian.com` URL as Observer; same content already existed under `guardian--richard-lawson.json`).
  - Cleared `originalScore` → `null` on 3 files where `scoreSource` was an aggregator (`high-noon-west-end-2025/times-uk`, `oliver-west-end-2024/{daily-mail,times-uk}`). Rating preserved in `aggregatorStars`.
- **Systematic guards that landed (from parallel session)** — `scripts/lib/commercial-sources.js` normalizer (coerces LLM-emitted `"other"`/`"website"` to `manual`, accepts null dates); cross-domain URL-vs-name outlet refinement in `review-file-writer.js` + `extract-dtli-reviews.js`; aggregator-aware score writes in `retry-pending-scores.js` + `recover-explicit-ratings.js` (checks existing `scoreSource`, not just result source); validator loosened to allow null `date` on commercial sources; all 3 LLM research prompts updated to include `manual` as a valid type.
- **Notion cards** — New: `Session wrap-up — Red Test Suite fix` (Done), `P1: undici critical DoS vulnerability — GHSA-phc3-fgpg-7m6h` (Not started → Done 2026-04-10, see undici cleanup entry above). Updated: existing aggregator-score card with addendum documenting the 3 additional files and the retry-pending-scores companion fix.

### matchTitleToShow Confidence Audit + Zero-Data Guards (2026-04-10/11)
- **12 aggregator scrapers hardened** — Same class of bug as WET: `matchTitleToShow()` returns medium-confidence fuzzy matches via word overlap, but callers accepted all matches. Added `confidence === 'high'` gates to: scrape-theatre-reviews, scrape-thestage-roundups, scrape-stagedoor-critics, scrape-playbill-verdict, scrape-nysr-reviews, scrape-wp-theater-blogs, discover-outlet-reviews-serp, scrape-london-box-office-roundups, sweep-we-aggregators, enrich-west-end-dates, broadway-com-runtimes, scrape-boring-waltz-costs. Plus backfill-grosses-history.ts caught in ship-check review.
- **Zero-data guards** — 6 scrapers now exit(1) on zero items fetched so silent failures surface within a cron cycle (WET was broken for ~2 weeks silently). Covers: WET, NYSR, WP theater blogs (sum of all sites), LBO roundups (sitemap URLs), The Stage (pages checked), Playbill Verdict (articles found).
- **LBO guard scope fix** — Initial zero-data guard referenced undefined `TARGET_SHOWS`; fixed to `targetShowIds` after ship-check audit.
- **sweep-we-aggregators fallback removed** — Word-overlap fallback after matchTitleToShow bypassed the new confidence gate ("Six" matched any slug with "six"). Removed entirely.
- **53 WET regression tests** at `tests/unit/wet-title-extraction.test.mjs` — covers 30+ real WP post title patterns + must-reject wrong-show cases.
- **scrape-new-aggregators.yml re-enabled** — Was manually disabled on 2026-04-03. No code reason remained (BrightData zone + checkout-core-data issues long resolved). Smoke-tested lbo-roundups job: 22 sitemap URLs → 14 high-confidence roundup matches + 50 individual reviews. Now back in weekly rotation (Sundays 11 AM UTC).
- **Ship-check fixes** — Independent code review found Playbill Verdict guard ordering bug (fired after Google fallback wasted API credits), unchecked `backfill-grosses-history.ts`, and dead code in NYSR + WP theater blogs. All fixed.
