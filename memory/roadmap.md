...
### Boy-at-back Oscillation Fixes + Poller/URL Protection (2026-04-10)
- **mergeReviews wrongProduction oscillation** — Opening-night-poller was wiping date-based wrongProduction flags every re-scrape, rebuild was re-setting them, loop repeated on every poller run. Concrete victim: the-boy-at-the-back-of-the-class-west-end-2026 (5 Show Score reviews from 2024 Rose Theatre Kingston tour). Commit `47702cbab3`: removed 'Pre-opening guard', 'Date guard', 'Dateless show' from mergeReviews auto-clear list (only 'Same URL' remains for venue-transfer self-heal); tightened URL-change clear to only URL-based flags; added existingIsDateBasedWrongProd guard to gather-reviews.js replacement branch. 8 new unit tests.
- **mergeReviews URL protection** — URL-change path was unconditionally replacing existing URLs with incoming. For boy-at-back thestage, this reverted manually-corrected long URL to known-dead short URL every poller run (url_dead_404 counter reached 38). Commit `c3a4bb1da9` + `51bdaccca7`: respect `urlVerified` and `urlManualOverride` as don't-overwrite markers; added to PROTECTED_FIELDS list; guard only blocks URL CHANGES (first-URL-set and undefined-repair still proceed). 5 new unit tests.
- **Stop-gap**: set `wrongProductionNote` prefix to 'Tour transfer:' on 5 boy-at-back show-score files (broadway-review-texts `1e4810c15e`) to survive pre-fix poller runs; re-applied urlManualOverride after pre-fix gather wiped it (`699fe0b364`). Stop-gap is now redundant with code fix but harmless to leave in place.
- **Touring-then-WE audit (Notion)** — Query run to find WE/OWE shows with unflagged reviews mentioning UK touring venues not in show venue. 7 candidates identified (soldiers-of-tomorrow, jeezus, oliver, kanpur-1857, marie-and-rosetta, copenhagen, this-is-not-about-me) + 2 false positives (hadestown, john-proctor). Full report + triage guidance in Notion row `33e637c5-416f-81aa-8f92-e9dceeebdcdb`.
