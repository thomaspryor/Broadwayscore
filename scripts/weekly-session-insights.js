 ...
 *
 * Scheduled via launchd: ~/Library/LaunchAgents/com.bwsc.weekly-session-insights.plist
 *
 ...
 *
 * Privacy: output contains raw user text and tool args. It lands in
 * ~/Documents/claude-outputs/ which iCloud-syncs to phone and macOS backups.
 * Secret-shape strings (Bearer/sk-/ghp_/AKIA/xoxb/api_key=) are best-effort
 * redacted before write. This is YOUR data going to YOUR iCloud; review any
 * digest before forwarding.
 *
 * Sunset rule: if four consecutive weekly runs produce zero findings, the
 * script PRINTS a reminder telling you to unload the plist. It does NOT
 * auto-unload (silent launchctl failures + permission edge cases are worse
 * than a stale cron). Act on the reminder manually.
 */
...
// Redact obvious secret shapes before writing to iCloud-synced disk.
// Output lands in ~/Documents/claude-outputs/ which syncs to phone + macOS
// backups + Spotlight. Failure mode: a session transcript that pasted an API
// key would otherwise land in iCloud. This is best-effort, not exhaustive.
const SECRET_PATTERNS = [
  [/\b(Bearer\s+)[A-Za-z0-9._\-]{20,}/g, '$1<REDACTED>'],
  [/\bsk-[A-Za-z0-9_\-]{20,}/g, 'sk-<REDACTED>'],
  [/\b(ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9]{30,}/g, '$1_<REDACTED>'],   // GitHub
  [/\bAKIA[0-9A-Z]{16}\b/g, 'AKIA<REDACTED>'],                       // AWS
  [/\b(xox[abopr])-[A-Za-z0-9-]{10,}/g, '$1-<REDACTED>'],            // Slack
...
function findCorrections(events, sessionId) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    out.push({
      ...
      user: scrub(text.trim().slice(0, 400)),
      context: ctx.map(x => ({ ...x, body: scrub(x.body) })),
    });
  }
  return out;
}
    ...
    for (const c of content) {
      ...
      for (const line of body.split('\n')) {
        ...
        if (m && line.length < 240) {
          out.push(scrub(line.trim().slice(0, 200)));
        }
      }
    }
  }
...
function main() {
  ...
  if (findingCount === 0) {
    ...
    if (state.zeroStreak >= 4) {
      ...
      console.log('   launchctl unload ~/Library/LaunchAgents/com.bwsc.weekly-session-insights.plist');
    }
    saveState(state);
  ...
  lines.push('');
  lines.push('```');
  lines.push('launchctl unload ~/Library/LaunchAgents/com.bwsc.weekly-session-insights.plist');
  lines.push('```');
