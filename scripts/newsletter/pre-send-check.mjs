...
//     - Featured opening whose gap audit shows uncollected reviews (score not
//       settled; waive once with NEWSLETTER_ALLOW_GAPS=1) — task #823
...
} = createRequire(import.meta.url)('../lib/newsletter-preflight.js');
...
// ── HARD: featured-opening images + review completeness (task #823) ──────────
...
// shows had uncollected aggregator-cited reviews (Brainiac gaps=3, Traitors
// Live gaps=7) — the displayed scores weren't settled. Both facts are known
// locally; the owner must never be the one catching them.
...
if (!Array.isArray(meta.openingShows)) {
  ...
  // Review completeness vs the hourly gap audit's per-show checkpoint.
  // Fresh entry with gaps>0 → HARD (score built on an incomplete review set);
  // stale/absent entry → SOFT (unverified, named in the banner).
  // NEWSLETTER_ALLOW_GAPS=1 downgrades the hard failures to banner warnings —
  // the owner's explicit, single-run waiver.
  let gapCheckpoint = {};
  ...
  for (const v of gapFindings.hard) {
    if (allowGaps) softIssues.push(`[waived via NEWSLETTER_ALLOW_GAPS=1] ${v}`);
    else hardFailures.push(v);
  }
  for (const v of gapFindings.soft) softIssues.push(v);
}
