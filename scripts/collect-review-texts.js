...
async function fetchWithBrightData(url) {
  ...
  try {
    ...
    if (isBlocked(html)) {
      recordBdCall({ url, fn: 'web-unlocker', success: false, status: 'blocked' });
    ...
    recordBdCall({ url, fn: 'web-unlocker', success: false, status: 'insufficient-text' });
    throw new Error(`Insufficient text: ${text?.length || 0} chars`);
  } catch (error) {
    if (error.response) {
      const status = error.response.status;
      recordBdCall({ url, fn: 'web-unlocker', success: false, status });
      const message = error.response.data?.message || error.response.data || error.message;
      throw new Error(`Bright Data error (${status}): ${message}`);
    }
    // Re-throws after already recording (blocked/insufficient-text) don't double-record
    throw error;
  }
}
...
function findReviewsToProcess() {
  ...
  // Also build staleClosedShowIds: closed shows whose closingDate is >180 days ago.
  // Reviews for these shows are either already collected or will never be collectible
  // from the live web — retrying BD/SB on them wastes budget.
  const STALE_CLOSED_DAYS = 180;
  const openShowIds = new Set();
  const showOpeningDates = new Map(); // showId → openingDate string
  const staleClosedShowIds = new Set();
  try {
    const showsData = JSON.parse(fs.readFileSync('data/shows.json', 'utf8'));
    const cutoff = Date.now() - STALE_CLOSED_DAYS * 24 * 60 * 60 * 1000;
    (showsData.shows || showsData).forEach(s => {
      ...
      if (s.status === 'closed' && s.closingDate) {
        if (new Date(s.closingDate).getTime() < cutoff) staleClosedShowIds.add(s.id);
      }
    });
    console.log(`  Prioritizing ${openShowIds.size} open/preview shows`);
    if (staleClosedShowIds.size > 0) {
      console.log(`  Skipping ${staleClosedShowIds.size} shows closed >${STALE_CLOSED_DAYS}d ago (lifecycle gate)`);
    }
  } catch (e) {
  ...
  for (const showId of shows) {
    ...
    // Skip shows that closed >180 days ago — their reviews are either already collected
    // or will never be collectible from the live web. Bypass when explicitly targeting
    // a show (showFilter / reviewFilter).
    if (staleClosedShowIds.has(showId) && CONFIG.reviewFilter.size === 0 && !CONFIG.showFilter) continue;
