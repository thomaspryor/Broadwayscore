...
async function checkSitemapStatus(token) {
  ...
  // Self-heal a stale root sitemap.xml submission. This site serves only sharded
  ...
  // persistent error in GSC. Delete it — but ONLY after a live fetch confirms it's
  // a dead orphan (non-200), so a valid sitemap is never removed. Real shards end
  // in /sitemap/N.xml and can't match the exact `/sitemap.xml` path.
  ...
  if (orphan && !dryRun) {
    try {
      const live = await fetch(orphanPath, { method: 'HEAD' });
      if (live.status !== 200) {
        const del = await fetch(
          `https://www.googleapis.com/webmasters/v3/sites/${siteUrl}/sitemaps/${encodeURIComponent(orphanPath)}`,
          { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }
        );
        if (del.ok) {
          orphanDeleted = true;
          console.log(`  Self-heal: deleted stale orphan sitemap ${orphanPath} (live HTTP ${live.status}, never generated by this site)`);
        } else {
          console.log(`  Self-heal: failed to delete ${orphanPath} — HTTP ${del.status}`);
        }
      } else {
        console.log(`  Note: ${orphanPath} has GSC errors but returns 200 live — leaving in place`);
      }
    } catch (err) {
      console.log(`  Self-heal: error checking/deleting orphan sitemap — ${err.message}`);
    }
  } else if (orphan && dryRun) {
    console.log(`  [Dry run] Would check/delete stale orphan sitemap ${orphanPath}`);
  }

  // If we just removed the orphan, drop it from the reported list so the snapshot
  // reflects post-heal reality immediately.
  const reported = orphanDeleted ? sitemaps.filter(sm => sm.path !== orphanPath) : sitemaps;

  return {
    orphanDeleted,
    count: reported.length,
    sitemaps: reported.map(sm => ({
      path: sm.path,
      errors: sm.errors || 0,
