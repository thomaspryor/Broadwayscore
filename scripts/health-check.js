...
// Send a focused, actionable email when the rebuild has been guard-blocked for 2+ days.
// This is separate from the daily digest — it fires once per 48h and tells you exactly
// what to paste into a Claude session to investigate and unblock.
async function sendGuardStallAlert(consecutiveErrorDays) {
  const apiKey = process.env.RESEND_API_KEY;
  const ownerEmail = process.env.OWNER_EMAIL;
  if (!apiKey || !ownerEmail) return;

  // 48h cooldown — don't spam on every daily health-check run
  const cooldownFile = path.join(TRIAGE_DIR, 'guard-stall-alert.json');
  try {
    if (fs.existsSync(cooldownFile)) {
      const state = JSON.parse(fs.readFileSync(cooldownFile, 'utf8'));
      const hoursSinceLast = (Date.now() - new Date(state.lastSent || 0).getTime()) / (1000 * 60 * 60);
      if (hoursSinceLast < 48) {
        console.log(`[Guard Stall Alert] Suppressed — last sent ${Math.round(hoursSinceLast)}h ago`);
        return;
      }
    }
  } catch {}

  // Read guard audit files for context
  const regressionFile = path.join(AUDIT_DIR, 'rebuild-regression.json');
  const driftFile = path.join(AUDIT_DIR, 'rebuild-show-drift.json');
  const lastSuccessFile = path.join(PIPELINE_DIR, 'rebuild-reviews.last-success');
  ...
  try {
    const r = JSON.parse(fs.readFileSync(regressionFile, 'utf8'));
    const count = (r.regressions || []).length;
    const guardTime = new Date(r.timestamp || 0).getTime();
    const lastSuccessTime = new Date(lastSuccess).getTime();
    if (guardTime > lastSuccessTime && count > 0) {
      guardNames.push('Regression Guard');
      const top5 = (r.regressions || []).slice(0, 5).map(s => `${s.showId} (−${s.dropped} reviews)`).join(', ');
      regressionSummary = `<p style="margin:8px 0;"><strong>Regression Guard:</strong> ${count} show(s) lost reviews. Top affected: ${top5}${count > 5 ? ` …and ${count - 5} more` : ''}.</p>`;
    }
  } catch {}

  try {
    const d = JSON.parse(fs.readFileSync(driftFile, 'utf8'));
    const count = (d.shows || []).length;
    const guardTime = new Date(d.timestamp || 0).getTime();
    const lastSuccessTime = new Date(lastSuccess).getTime();
    if (guardTime > lastSuccessTime && count > 0) {
      guardNames.push('Drift Guard');
      const top5 = (d.shows || []).slice(0, 5).map(s => `${s.showId} (${s.delta > 0 ? '+' : ''}${Math.round(s.delta)}pts)`).join(', ');
      driftSummary = `<p style="margin:8px 0;"><strong>Drift Guard:</strong> ${count} show(s) with score shift. Top affected: ${top5}${count > 5 ? ` …and ${count - 5} more` : ''}.</p>`;
    }
  } catch {}

  if (guardNames.length === 0) return; // Not a guard-blocked stall — skip

  const guardLabel = guardNames.join(' + ');
  const claudePrompt = `The Broadway Scorecard rebuild pipeline has been stalled for ${consecutiveErrorDays} days. The ${guardLabel} is blocking the daily rebuild.

Last successful rebuild: ${lastSuccess}

Please investigate:
1. Check data/audit/rebuild-regression.json and data/audit/rebuild-show-drift.json
2. Determine if the drops/drift are from intentional cleanup (dedup, flagging) or a real data problem
3. If safe, unblock with: gh workflow run "Rebuild Reviews Data" -f allow_regression=true -f reason="Manual recovery after ${consecutiveErrorDays}-day stall"
4. If suspicious, investigate which shows are affected before unblocking`;

  const html = `<!DOCTYPE html><html><body style="background:#1a1a1a;color:#ddd;font-family:sans-serif;padding:24px;max-width:600px;">
<h2 style="color:#e74c3c;margin:0 0 4px;">&#128680; Rebuild Stalled: ${consecutiveErrorDays} Days</h2>
<p style="color:#aaa;margin:0 0 16px;font-size:13px;">The ${guardLabel} has blocked the daily rebuild for ${consecutiveErrorDays} consecutive days. Reviews collected since ${lastSuccess} have not been published.</p>

<div style="background:#2a1a1a;border-left:3px solid #e74c3c;padding:12px 16px;margin-bottom:16px;border-radius:4px;">
${regressionSummary}${driftSummary}
</div>

<h3 style="color:#f39c12;margin:0 0 8px;font-size:14px;">TO INVESTIGATE: Paste this into a new Claude Code session</h3>
<div style="background:#111;border:1px solid #444;border-radius:4px;padding:12px;margin-bottom:16px;">
<pre style="margin:0;white-space:pre-wrap;font-size:12px;color:#adf;line-height:1.5;">${claudePrompt}</pre>
</div>

<h3 style="color:#2ecc71;margin:0 0 8px;font-size:14px;">TO UNBLOCK IMMEDIATELY (if you trust the drops are safe)</h3>
<div style="background:#111;border:1px solid #444;border-radius:4px;padding:12px;">
<pre style="margin:0;font-size:12px;color:#adf;">gh workflow run "Rebuild Reviews Data" -f allow_regression=true -f reason="Manual recovery"</pre>
</div>

<p style="color:#666;font-size:11px;margin:16px 0 0;">This alert fires once per 48h while the stall persists. Daily digest has full context.</p>
</body></html>`;
  ...
  return new Promise((resolve) => {
    const data = JSON.stringify({
      ...
      to: [ownerEmail],
      subject: `BSC Action Required: Rebuild stalled ${consecutiveErrorDays} days (${guardLabel})`,
      html,
    });
    const req = require('https').request({
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          console.log('[Guard Stall Alert] Sent successfully');
          // Record send time for cooldown
          try {
            fs.mkdirSync(path.dirname(cooldownFile), { recursive: true });
            fs.writeFileSync(cooldownFile, JSON.stringify({ lastSent: new Date().toISOString(), days: consecutiveErrorDays }) + '\n');
          } catch {}
        } else {
          console.error(`[Guard Stall Alert] Failed: ${res.statusCode} ${body}`);
        }
        resolve();
      });
    });
    req.on('error', (err) => { console.error('[Guard Stall Alert] Error:', err.message); resolve(); });
    req.end(data);
  });
}
...
async function main() {
  ...
  // Guard stall alert: dedicated actionable email when rebuild has been blocked 2+ days.
  // Fires separately from the digest (once per 48h) with Claude prompt + unblock command.
  if (history.consecutiveErrorDays >= 2) {
    await sendGuardStallAlert(history.consecutiveErrorDays);
  }
