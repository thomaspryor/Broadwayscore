...
try {
  ...
  console.warn('⚠ shows.json not found — generating empty off-broadway-archive.json');
}
...
function computeCriticScore(showReviews) {
  const result = _computeRaw(showReviews, outletRegistry, 'off-broadway');
...
// Closed Off-Broadway shows with 5+ scored reviews
...
const archiveData = archiveShows.map(show => {
  ...
  const entry = {
    ...
    // explicitly here, mirroring serializeShow's { category: 'off-broadway' }
    // override for the page's active shows.
...
const outputPath = path.join(outputDir, 'off-broadway-archive.json');
...
console.log(`✓ Generated off-broadway-archive.json: ${archiveData.length} closed shows (${sizeKB}KB)`);
