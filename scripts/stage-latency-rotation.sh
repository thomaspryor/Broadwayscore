...
git add "$LOG_FILE"
# --only (BRO-3212 /what-else cousin fix): this script's only caller is
# .github/workflows/stage-latency-rotation.yml, a fresh actions/checkout@v5
# each run — not the shared local ~/Broadwayscore checkout sync-audit-
# checkout.sh's identical fix (scripts/lib/sync-audit-checkout.sh) guards
# against, so there is no concurrent-writer risk here today. `--only`
# restricts the commit to exactly $LOG_FILE regardless of what else might
# be sitting in the index — cheap defense-in-depth that costs nothing when
# nothing else is staged (the normal case), and keeps this script correct
# if it's ever run against a shared checkout or gains a second staged file.
