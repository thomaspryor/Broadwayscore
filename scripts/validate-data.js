...
const { isBroadwayCategory, isNonNycLocale } = require('./lib/venue-classification');
...
function validateStatus(shows) {
  ...
  for (const show of shows) {
    ...
    if ((show.category === 'broadway' || show.category === 'off-broadway')
        && show.venue && isNonNycLocale(show.venue)) {
      error(`Show "${show.title}" (${show.id}) has category="${show.category}" but venue "${show.venue}" is outside New York — Broadway and Off-Broadway are NYC designations, so this is a mis-categorised touring/regional date. Remove the row or recategorise it (category="regional"), and fix the creator: if it came from TodayTix, add the venue to NON_NYC_VENUE_RE in scripts/lib/venue-classification.js so discovery stops re-minting it.`);
      invalid++;
    }
