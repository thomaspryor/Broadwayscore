...
function revertFile(filePath) {
  try {
    execSync(`git checkout -- "${filePath}"`, { cwd: ROOT, stdio: 'pipe' });
  } catch {
    // best effort
  }
}
...
async function main() {
  ...
  const cards = await getPendingFrictionCards(notion);
  ...
  for (const card of toProcess) {
    ...
    const commitMsg = `fix(auto-fix/${fhash}): ${title.slice(0, 60)}`;
