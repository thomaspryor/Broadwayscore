#!/usr/bin/env node
/**
 * Bundle the aggregate-catalog screenshots + spec into a single PDF.
 *
 * Reads from ~/Documents/claude-outputs/aggregate-catalog/
 *   (manifest.json + *.png + SPEC.md)
 * Outputs:
 *   ~/Documents/claude-outputs/aggregate-catalog/aggregate-catalog.pdf
 *
 * Mirror of bundle-show-page-catalog-pdf.mjs — same layout, different I/O dir
 * and different entry key (entry.path instead of entry.slug).
 */
...
const DIR = join(homedir(), 'Documents/claude-outputs/aggregate-catalog');
const COMPRESSED_DIR = join(DIR, '_compressed');
const OUT = join(DIR, 'aggregate-catalog.pdf');
...
async function main() {
  ...
  parts.push(`<h1>Aggregate Page Catalog</h1>`);
  ...
  for (const g of groups) {
    ...
    for (const e of g.entries) {
      ...
      parts.push(`<h3>${escapeHtml(e.label)} <span style="color:#888;font-weight:400;font-family:ui-monospace,monospace;font-size:13px;">${escapeHtml(e.path)}</span></h3>`);
