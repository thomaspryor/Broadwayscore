#!/usr/bin/env node
/**
 * Aggregate Page Catalog — screenshot every curated aggregate / list / table
 * URL at desktop + mobile.
 *
 * Reads paths from src/app/dev/aggregate-catalog/catalog-data.mjs.
 * Drives Playwright against http://localhost:3000 (override with
 * CATALOG_BASE_URL).
 *
 * Output: ~/Documents/claude-outputs/aggregate-catalog/
 *
 * Usage:
 *   npm run dev                                   # terminal 1
 *   node scripts/capture-aggregate-catalog.mjs    # terminal 2
 */
...
const CATALOG_SRC = join(REPO_ROOT, 'src/app/dev/aggregate-catalog/catalog-data.mjs');
const CATALOG_INDEX_PATH = '/dev/aggregate-catalog';
...
const OUT_DIR = flagVal('--out') || join(homedir(), 'Documents/claude-outputs/aggregate-catalog');
...
async function capture() {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const pathKey = fileSafe(entry.path);
    const stem = `${String(groupIdx).padStart(2, '0')}-${String(i + 1).padStart(2, '0')}-${pathKey}`;
  ...
  for (const vp of VIEWPORTS) {
    ...
    const page = await ctx.newPage();
    try {
      await page.goto(`${BASE_URL}${CATALOG_INDEX_PATH}`, { waitUntil: 'networkidle', timeout: 30_000 });
      await page.waitForTimeout(300);
      await page.screenshot({
