...
ensure_notion_client() {
  ...
  if [ -n "${NOTION_API_KEY:-}" ] && ! node -e "require('@notionhq/client')" >/dev/null 2>&1; then
    ...
    npm install --no-save @notionhq/client >/dev/null 2>&1 \
      && log "installed @notionhq/client" \
      || log "WARN: could not install @notionhq/client (notion-brain.js will fail)"
  fi
}
...
# 2. Try the real private repos if we have any way to authenticate.
if [ -n "${REVIEW_TEXTS_TOKEN:-}" ] || command -v gh >/dev/null 2>&1; then
  log "attempting real data via setup-local-data.sh"
  bash "$SCRIPT_DIR/setup-local-data.sh" 2>&1 | sed 's/^/[setup-local-data] /' || true
fi
