...
const {
  evaluateIntake, countCreatedToday, recordCreated, dayKey, DEFAULT_DAILY_CAP,
...
test('countCreatedToday: counts only same-UTC-day entries', () => {
  const led = tmpLedger();
...
test('countCreatedToday: a line merely CONTAINING the date string is not counted without a valid ts', () => {
  // The cheap substring prefilter must not become the decision — a title like
  // "fix the 2026-08-13 regression" would otherwise inflate the count.
  const led = tmpLedger();
