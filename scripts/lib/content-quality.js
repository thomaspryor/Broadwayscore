...
function extractAuthorFromHtml(html, text, options = {}) {
  ...
  // Outlet override: thestage.co.uk prints the critic as an inline anchor
  // right under the headline — <a class="aos-ArticleAuthor ..." title="Holly
  // O'Mahony" href="/hollyom">by&nbsp;Holly O'Mahony</a>. The title attribute
  // is per-article (not a shared site account), so it's a reliable signal,
  // but it's HTML-entity-encoded (numeric entities like &#x27;) which the
  // generic byline patterns below don't decode — decode before validating.
  // Attribute order isn't assumed (capture the whole anchor, pull title from
  // within it) since a template/cache variant could emit title before class.
  // BRO-171 (545 Unknown-critic recovery sweep).
  if (options && options.url && /(^|\/\/|\.)thestage\.co\.uk/i.test(String(options.url))) {
    const tsAnchor = /<a\b[^>]*\baos-ArticleAuthor\b[^>]*>/.exec(html);
    const tsTitle = tsAnchor && /\btitle="([^"]+)"/.exec(tsAnchor[0]);
    if (tsTitle) {
      const { decodeHtmlEntities } = require('./text-cleaning');
      const decoded = decodeHtmlEntities(tsTitle[1]).replace(/\s+/g, ' ').trim();
      if (isValidAuthorName(decoded)) return cleanAuthorName(decoded);
    }
  }

  // Outlet override: theatermania.com's JSON-LD only references the author by
  // an opaque WP username slug ("@id":".../author/phempstead/#author") — the
  // display name is never inlined on the page. Resolve the slug against the
  // outlet's known critic roster (data/critic-registry.json) using ONLY an
  // unambiguous "first-initial + lastname" or "firstname" match; anything
  // else is left unresolved rather than guessed — a wrong default critic
  // previously misattributed 30/31 Plays International reviews (2026-08-03).
  // BRO-171.
  if (options && options.url && /(^|\/\/|\.)theatermania\.com/i.test(String(options.url))) {
    const tm = resolveTheaterManiaByline(html);
    if (tm) return tm;
  }
  ...
  const bylinePatterns = [
    ...
    // Token separator is [ \t]+ (not \s+) so a blank-line-separated dateline
    // that follows the byline on some pages ("Thomas Burke \n\n NEW YORK -
    // March 10, 2000") can't be swallowed into the name capture — BRO-171.
    /Theatre Review by[ \t]+(?:<a\b[^>]*>\s*)?([A-Z][A-Za-zÀ-ÿ'’\-]+(?:[ \t]+[A-Z][A-Za-zÀ-ÿ'’\-]+){0,3})(?:\s*<\/a>)?/,
  ];
...
// data/critic-registry.json is gitignored (generated by
// audit-critic-outlets.js) and may not exist on a fresh checkout — cache +
// fail-safe to an empty roster rather than letting a bare require() throw and
// take down the whole extraction call for that review. Mirrors
// loadBroadwayShows()'s shape above.
let _cachedTheaterManiaCritics = null;

function loadTheaterManiaCritics() {
  if (_cachedTheaterManiaCritics) return _cachedTheaterManiaCritics;
  try {
    const fs = require('fs');
    const registryPath = require('path').join(__dirname, '../../data/critic-registry.json');
    const registry = JSON.parse(fs.readFileSync(registryPath, 'utf-8'));
    _cachedTheaterManiaCritics = Object.values(registry.critics || {})
      .filter((c) => c.displayName && c.displayName !== 'Unknown' && (c.knownOutlets || []).includes('theatermania'))
      .map((c) => c.displayName);
  } catch {
...
/**
 * TheaterMania: resolve the WP author slug referenced by the article's
 * JSON-LD (no inline display name on the page) against the outlet's known
 * critic roster. Only an unambiguous first-initial+lastname or bare-firstname
 * match is accepted; anything else returns null rather than a guess.
 * @param {string} html live-fetched article HTML
 * @returns {string | null}
 */
function resolveTheaterManiaByline(html) {
  const idMatch = /"author"\s*:\s*\{\s*"@id"\s*:\s*"([^"]+)#author"\s*\}/.exec(html);
  if (!idMatch) return null;
  const authorUrl = idMatch[1].replace(/\\\//g, '/');
  const slugMatch = /\/author\/([a-z0-9-]+)\/?$/i.exec(authorUrl);
  if (!slugMatch) return null;
  return matchTheaterManiaSlug(slugMatch[1].toLowerCase(), loadTheaterManiaCritics());
}

/**
 * Pure matcher: WP author slug + known-critic roster -> unambiguous name, or
 * null. Split out from resolveTheaterManiaByline so it's testable without the
 * gitignored data/critic-registry.json on disk.
 * @param {string} slug lowercased WP username slug, e.g. "phempstead"
 * @param {string[]} critics candidate display names, e.g. ["Pete Hempstead"]
 * @returns {string | null}
 */
function matchTheaterManiaSlug(slug, critics) {
  const matches = (critics || []).filter((name) => {
    const parts = name.trim().split(/\s+/);
    if (parts.length < 2) return false;
    const first = parts[0].toLowerCase().replace(/[^a-z0-9]/g, '');
    const last = parts[parts.length - 1].toLowerCase().replace(/[^a-z0-9]/g, '');
    return slug === first[0] + last || slug === first;
  });
