...
test('meta with no extractedAt/extractedAtUnix -> fail', () => {
  const r = checkWsjOtpFreshness({}, NOW);
  assert.equal(r.status, 'fail');
});
...
test('just under WARN threshold -> pass', () => {
  const r = checkWsjOtpFreshness({ extractedAtUnix: NOW - (WSJ_OTP_WARN_DAYS - 1) * 86400 }, NOW);
  assert.equal(r.status, 'pass');
});

test('exactly at WARN threshold (40d) -> warn', () => {
  const r = checkWsjOtpFreshness({ extractedAtUnix: NOW - WSJ_OTP_WARN_DAYS * 86400 }, NOW);
  assert.equal(r.status, 'warn');
  assert.equal(r.ageDays, WSJ_OTP_WARN_DAYS);
});

test('between WARN and FAIL -> warn', () => {
  const r = checkWsjOtpFreshness({ extractedAtUnix: NOW - 55 * 86400 }, NOW);
  assert.equal(r.status, 'warn');
});

test('exactly at FAIL threshold (70d) -> fail', () => {
  const r = checkWsjOtpFreshness({ extractedAtUnix: NOW - WSJ_OTP_FAIL_DAYS * 86400 }, NOW);
  assert.equal(r.status, 'fail');
  assert.match(r.message, /launchd job likely dead/);
});

test('way past FAIL -> fail', () => {
  const r = checkWsjOtpFreshness({ extractedAtUnix: NOW - 200 * 86400 }, NOW);
  assert.equal(r.status, 'fail');
});

test('extractedAt ISO string fallback (no extractedAtUnix) is honored', () => {
  const isoDate = new Date((NOW - 5 * 86400) * 1000).toISOString();
  const r = checkWsjOtpFreshness({ extractedAt: isoDate }, NOW);
  assert.equal(r.status, 'pass');
  assert.equal(r.ageDays, 5);
});

test('future extractedAt (clock skew / corrupt meta) -> warn, not pass or fail', () => {
  const r = checkWsjOtpFreshness({ extractedAtUnix: NOW + 2 * 86400 }, NOW);
  assert.equal(r.status, 'warn');
