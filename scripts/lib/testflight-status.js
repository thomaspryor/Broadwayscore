...
async function getBuildBetaStatus(version) {
  ...
  // filter[version] is not a supported query param on this collection —
  // page through builds and match the build number client-side instead.
  const builds = await ascGet(`/v1/builds?filter[app]=${app.id}&fields[builds]=version&limit=200`);
  const build = builds.data?.find((b) => b.attributes.version === String(version));
  ...
  const [detail, submission, groups] = await Promise.all([
    ...
    ascGet(`/v1/apps/${app.id}/betaGroups?fields[betaGroups]=name,isInternalGroup,publicLinkEnabled,publicLink`),
  ]);
  ...
  const publicGroup = groups.data?.find((g) => !g.attributes.isInternalGroup);
...
module.exports = { hasCredentials, keyPath, getBuildBetaStatus, SUBMITTED_STATES };
