/**
 * Task-store archiver — moves long-completed tasks out of the live
 * ~/.claude/tasks/<list>/ directory into a sibling archive/ subdirectory,
 * so the harness's injected task-list reminder (one line per file in the
 * live directory, ~860 lines / avg 16x per session per the 5-day audit)
 * shrinks to just pending/in_progress/recently-completed work.
 *
 ...
 * status — old completed tasks (the only archive candidates) are always
 * far below the current id frontier in practice, so this costs nothing.
 *
 * Task JSON carries no completion timestamp (checked: real corpus has
 * none), so "completed >48h ago" is judged off the file's own mtime —
 * the last write is the status flip to completed in the overwhelming
 * common case.
 */
 ...
 */
function selectArchivable(tasks, { now, maxAgeMs = DEFAULT_MAX_AGE_MS, keepTopN = DEFAULT_KEEP_TOP_N }) {
  ...
  return tasks
    ...
    .filter((t) => !frontier.has(t.id))
    .filter((t) => typeof t.mtimeMs === 'number' && now - t.mtimeMs > maxAgeMs)
    .map((t) => t.id)
...
function archiveCompletedTasks(dir, opts = {}) {
  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (parsed.status !== 'completed') { skipped.push({ id, reason: `status changed to ${parsed.status}` }); continue; }
