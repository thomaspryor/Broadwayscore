/**
 * Pure decision logic for the "no push-to-main workflow may silently cancel
 * main commits" guard (scripts/audit-workflow-concurrency.js).
 *
...
// Returns { group } if this workflow's raw YAML text would collapse main
// commits into a shared concurrency group with cancel-in-progress: true (the
// bug class described above). Returns null if the workflow is safe: no
// push-to-main trigger, no concurrency block, a per-run group, a non-`true`
// cancel-in-progress, or an explicit opt-out annotation.
function findConcurrencyViolation(raw) {
  if (!triggersOnMainPush(raw)) return null;

  const c = readConcurrency(raw);
  if (!c) return null; // no concurrency block → nothing cancels

  // Only the literal `true` is a problem. `false` and a `${{ ... }}` expression pass.
  if (c.cancelRaw !== 'true') return null;

  const isPerRun = PER_RUN_TOKENS.some((t) => c.group.includes(t));
  if (isPerRun) return null; // unique per run → never collapses commits

  // Explicit opt-out annotation inside the concurrency block.
  if (c.blockText.includes(ANNOTATION)) return null;
