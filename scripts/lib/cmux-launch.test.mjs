...
// whole session (card #548).
const PS_SAMPLE = `/sbin/launchd
bash /var/folders/__/n5f8n1yj2wnch4lpmz1138840000gn/T/bsc-cmd-545.sh
/Users/tompryor/.local/bin/claude --session-id abc --model sonnet --dangerously-skip-permissions [#545] Retrofit hasHelpFlag —
bash /var/folders/__/n5f8n1yj2wnch4lpmz1138840000gn/T/bsc-cmd-548.sh
...
test('hasSeedProcess: finds the bash wrapper for a live seedKey', () => {
  assert.equal(hasSeedProcess(PS_SAMPLE, '545'), true);
  assert.equal(hasSeedProcess(PS_SAMPLE, '548'), true);
});

test('hasSeedProcess: false for a seedKey with no matching wrapper (the #548 false-positive case)', () => {
  ...
  assert.equal(hasSeedProcess(PS_SAMPLE, '999'), false);
});

test('hasSeedProcess: numeric-prefix seedKeys do not collide (5 vs 545, 54 vs 545)', () => {
  assert.equal(hasSeedProcess(PS_SAMPLE, '5'), false);
  assert.equal(hasSeedProcess(PS_SAMPLE, '54'), false);
  assert.equal(hasSeedProcess(PS_SAMPLE, '5455'), false);
});

test('hasSeedProcess: empty/garbage ps output is never a false positive', () => {
  assert.equal(hasSeedProcess('', '545'), false);
  assert.equal(hasSeedProcess('not a process list', '545'), false);
});
