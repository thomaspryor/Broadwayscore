...
- **Critic Score = `compositeScore` from the engine, integer-rounded for display.** Any analysis or external claim ("Lost Boys scored X") MUST use `getCriticScore(showId)` from `scripts/lib/canonical-critic-scores.ts`, which returns the same tier-weighted number the site displays (T1 outlets 1.0, T2 0.75, T3 0.35), rounded to integer. **Never compute your own mean of `assignedScore` from reviews.json** — that's a different, non-canonical number. JS callers can dump via `npx tsx scripts/lib/canonical-critic-scores.ts > /tmp/critic-scores.json` then read the JSON. Near-miss 2026-05-30: a Reddit post about to ship with raw-mean numbers that didn't match the site.
