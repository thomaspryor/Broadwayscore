...
**TIMING RULE — read before anything else:** Aggregator/outlet review pages (BWW Review Roundup, DTLI, Talkin' Broadway, Playbill Verdict, Show Score, NYC Theatre Roundups, WET, theatre.reviews, Stagedoor, The Stage roundups) **do not exist until after opening night reviews start dropping.** They are built AS the reviews post, not before. You cannot pre-stage them, cannot pre-find URLs, cannot add a DTLI slug for a show that hasn't opened. A 404 today is normal — the page will exist tomorrow after the embargo lifts. **Do not offer to "pre-stage" these URLs. Do not treat their absence pre-opening as a gap.** Every session gets tripped up by this — the automation is designed to discover these URLs at poll time, after they're published. Only revisit items 6/8/9 below AFTER the first wave of Broadway reviews has landed in `reviews.json`.
...
6. **DTLI slug verification (Broadway only — DTLI doesn't cover WE):** `node -e "const m=require('./data/dtli-slug-map.json'); console.log('DTLI slug:', m.shows['{show-id}'])"` — verify slug maps to the current-season DTLI page, not an old production (e.g., `giant-2` not `giant`). For revival shows (year in ID), expect a numbered suffix. **Only valid AFTER opening — DTLI pages don't exist pre-opening.** Missing entry for a show that hasn't opened is expected, not a gap.
...
8. **BWW RR URL (Broadway primarily — rare for WE):** BWW only publishes the Review Roundup AFTER reviews start dropping — the URL does not exist pre-opening. Once published, grab it and pass as `--bww-roundup-url` to the opening-night-poller. Pattern: `https://www.broadwayworld.com/article/Review-Roundup-{TITLE-SLUG}-Opens-on-Broadway-{YYYYMMDD}`. If Google hasn't indexed it yet on same-day openings, the SERP fallback may return the wrong production — that's when the manual URL matters.
9. **Talkin' Broadway URL (Broadway only — TB doesn't review WE shows):** TB is an outlet (not an aggregator). Published AFTER opening night — do not try to pre-stage. Once live, direct URL pattern is `https://www.talkinbroadway.com/page/world/{titleslug}{year}.html` (e.g., `giant2026.html`). SERP returns forum posts (All That Chat) instead of the review. TB is T2 outlet — don't miss it once it lands.
