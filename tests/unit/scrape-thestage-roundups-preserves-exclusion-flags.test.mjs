...
//
// scrape-thestage-roundups.js's writeReviewFiles() looks up an existing file
...
const { writeReviewFiles } = require('../../scripts/scrape-thestage-roundups.js');
...
function flaggedOnDisk() {
  return {
    ...
    url: 'https://thetimes.co.uk/article/some-we-show-review',
...
function withTempDir(fn) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'thestage-roundups-flags-'));
  try {
    return fn(dir);
  } finally {
...
test('re-scraping The Stage roundup does not clobber a wrongProduction-flagged review', () => {
  withTempDir((dir) => {
    const showDir = path.join(dir, SHOW_ID);
    fs.mkdirSync(showDir, { recursive: true });
    const filePath = path.join(showDir, 'the-times--some-critic.json');
    fs.writeFileSync(filePath, JSON.stringify(flaggedOnDisk(), null, 2));

    writeReviewFiles(
      [{ outlet: 'The Times', outletId: 'the-times', critic: 'Some Critic', stars: 3, starsOutOf: 5, excerpt: 'A fresh excerpt.', url: null }],
      SHOW_ID,
      dir,
    );
...
test('a brand-new The Stage roundup file still gets created', () => {
  withTempDir((dir) => {
    writeReviewFiles(
