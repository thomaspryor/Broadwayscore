 ...
 * "Commit applied data" step. Pins the exact shipped value, not a loose
 * "<90" bound (rejected in review on the original fix, commit 0b81edfabe6).
 */
test('commercial-friday push step overrides GIT_NET_TIMEOUT_SEC to the shipped 30s value', () => {
  const step = findStep(loadWorkflow('commercial-friday.yml'), 'recoupment-scan', 'Commit applied data');
  const env = step.env || {};
...
test('commercial-friday push step still sets its existing PUSH_DEADLINE_SEC override', () => {
  const step = findStep(loadWorkflow('commercial-friday.yml'), 'recoupment-scan', 'Commit applied data');
  const env = step.env || {};
