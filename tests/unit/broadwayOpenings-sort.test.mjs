...
const { compareOpeningStories, sortOpeningStoriesByNewsworthiness } =
  require('../../scripts/lib/opening-story-order.js');

// Stand-in for generate.mjs's real isGoldTier(score, 'broadway') (gold min 83
// for Broadway) — the comparator takes a caller-supplied predicate rather
// than owning its own threshold, so production and this test each plug in
// their own gold check instead of duplicating the same magic number.
const GOLD_MIN = 83;
const isGold = (agg) => typeof agg?.avg === 'number' && agg.avg >= GOLD_MIN;
...
test('a gold-tier opening sorts ahead of a non-gold opening regardless of insertion order', () => {
  ...
  const sorted = sortOpeningStoriesByNewsworthiness(events, e => e.agg, isGold);
...
test('compareOpeningStories treats a missing aggregate as non-gold and lowest-ranked', () => {
  ...
  assert.ok(compareOpeningStories(gold, null, isGold) < 0, 'gold show must sort before a missing aggregate');
  assert.ok(compareOpeningStories(null, gold, isGold) > 0);
});
