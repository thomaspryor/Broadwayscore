 ...
 * "Commit audit ledger" step. Pins the exact shipped value, not a loose
 * "<90" bound (rejected in review on the original fix, commit 0b81edfabe6).
 */
test('audit-imageless-scored-shows push step overrides GIT_NET_TIMEOUT_SEC to the shipped 30s value', () => {
  const step = findStep(loadWorkflow('audit-imageless-scored-shows.yml'), 'audit', 'Commit audit ledger');
  const env = step.env || {};
...
test('audit-imageless-scored-shows push step still sets its existing PUSH_DEADLINE_SEC override', () => {
  const step = findStep(loadWorkflow('audit-imageless-scored-shows.yml'), 'audit', 'Commit audit ledger');
  const env = step.env || {};
