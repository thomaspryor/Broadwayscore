...
// ═══════════════════════════════════════════════════════════════
// SECTION 4: Show page rating layout checks
// Catches: misaligned icons, broken "Add date" position, inconsistent spacing
// ═══════════════════════════════════════════════════════════════

test.describe('Show page rating — mobile (390px)', () => {
  test.beforeEach(async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
  });

  test('previous viewings have consistent icon alignment', async ({ page }) => {
    await page.goto('/test/show-rating-fixture?state=multi');
    await page.waitForSelector('[data-testid="show-rating-fixture"]', { timeout: 30000 });

    const result = await page.evaluate(() => {
      // Find all "Edit this viewing" buttons (previous viewings)
      const editButtons = document.querySelectorAll('button[aria-label="Edit this viewing"]');
      if (editButtons.length < 2) return { hasEnoughViewings: false, maxXDiff: 0 };

      const xPositions = Array.from(editButtons).map(btn => btn.getBoundingClientRect().x);
      const maxX = Math.max(...xPositions);
      const minX = Math.min(...xPositions);

      return {
        hasEnoughViewings: true,
        maxXDiff: Math.round(maxX - minX),
        positions: xPositions.map(x => Math.round(x)),
      };
    });

    if (result.hasEnoughViewings) {
      expect(
        result.maxXDiff,
        `Edit icons misaligned by ${result.maxXDiff}px across viewings (positions: ${result.positions?.join(', ')})`
      ).toBeLessThan(15);
    }
  });

  test('watchlist date/link row is positioned below rating section', async ({ page }) => {
    await page.goto('/test/show-rating-fixture?state=existing');
    await page.waitForSelector('[data-testid="show-rating-fixture"]', { timeout: 30000 });

    const result = await page.evaluate(() => {
      const seeWatchlist = document.querySelector('a[href="/my-shows?tab=watchlist"]');
      if (!seeWatchlist) return { found: false };

      const ratingSection = seeWatchlist.closest('div.mt-5');
      if (!ratingSection) return { found: false };

      // Check that "See Watchlist" is BELOW the stars, not floating to the side
      const stars = ratingSection.querySelector('[role="radiogroup"]');
      if (!stars) return { found: false };

      const starsRect = stars.getBoundingClientRect();
      const linkRect = seeWatchlist.getBoundingClientRect();

      return {
        found: true,
        starsBottom: Math.round(starsRect.bottom),
        linkTop: Math.round(linkRect.top),
        isBelowStars: linkRect.top > starsRect.bottom,
      };
    });

    if (result.found) {
      expect(result.isBelowStars, `"See Watchlist" link (y=${result.linkTop}) is not below stars (bottom=${result.starsBottom})`).toBe(true);
    }
  });
});
