...
export default function GoldListSeasonPage({ params }: { params: { listType: string; season: string } }) {
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: season, url: `${BASE_URL}/lists/${listType}/${season}` },
  ]);
  ...
  return (
    <div className="min-h-screen bg-surface">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify([breadcrumbSchema, itemListSchema, generateGoldListFAQSchema(config, { season, entryCount: entries.length, topShow: entries[0]?.title })]) }}
      />
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-300 leading-relaxed">
            {config.description}
            {isCurrent && <span className="ml-2 text-xs text-amber-400 font-medium">(In Progress)</span>}
          </p>
          <p className="text-gray-500 text-sm mt-3">
            {entries.length} {entries.length === 1 ? 'show' : 'shows'} | {season} Season
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 mb-6">
          <Link
            ...
            className="flex items-center px-3 py-1.5 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/10 border border-transparent transition-colors"
          >
            All-Time
          </Link>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-1.5 truncate">
                    {(entry.status === 'previews' || entry.status === 'upcoming') && entry.openingDate ? (
                      ...
                      <>
                        <span className={entry.status === 'closed' ? 'text-gray-500' : 'text-amber-400'}>
                          {entry.status === 'closed' ? 'Closed' : 'Closes'} {formatGoldListDate(entry.closingDate)}
                        </span>
                        {entry.openingDate && (
