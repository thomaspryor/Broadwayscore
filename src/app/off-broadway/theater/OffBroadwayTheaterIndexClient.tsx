...
interface OffBroadwayTheaterSummary {
  name: string;
  slug: string;
  address?: string;
  showCount: number;
...
function TheaterCard({ theater }: { theater: OffBroadwayTheaterSummary }) {
  return (
    <Link
      href={`/off-broadway/theater/${theater.slug}`}
      className="card p-3 sm:p-4 flex items-center gap-3 sm:gap-4 hover:bg-surface-raised/80 transition-colors group"
    >
      {/* Placeholder icon (no exterior photos for off-Broadway venues yet) */}
...
export default function OffBroadwayTheaterIndexClient({ theaters }: { theaters: OffBroadwayTheaterSummary[] }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mb-6">
        <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2">Off-Broadway Theaters</h1>
        <p className="text-gray-400">
          {theaters.length} theaters · {playingCount} with shows currently running
        </p>
        <p className="text-xs text-gray-500 mt-1">Off-Broadway venues across New York City. Venue details (capacity, accessibility, seating tips) coming soon.</p>
      </div>
      ...
      <div className="relative mb-4" role="search">
        ...
        <input
          type="text"
          aria-label="Search Off-Broadway theaters or shows"
