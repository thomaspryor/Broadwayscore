Challenge an idea or proposed approach BEFORE investing in detailed planning. Catches "wrong problem" and "wrong approach" errors that technical critique cannot catch.

## When to use

Run `/sanity-check` BEFORE writing a detailed plan or using `/critique`. The workflow is:
1. Idea → `/sanity-check` → Validated approach
2. Validated approach → Plan → `/critique` → Refined plan → Implement

`/critique` finds implementation bugs. `/sanity-check` finds wrong-approach bugs. Both are needed for high-stakes work.
...
**Before writing, clear any stale files:** Delete `/tmp/sanity-check-problem.txt` and `/tmp/sanity-check-problem-only.txt` if they exist from a previous run.

Write this analysis to `/tmp/sanity-check-problem.txt`.
...
Append these approaches to `/tmp/sanity-check-problem.txt`.

**Also write a problem-only version** (WITHOUT the approaches) to `/tmp/sanity-check-problem-only.txt` — this is what GPT-4o gets, so it generates alternatives from scratch without being anchored.
...
1. **GPT-4o — Challenger & Alternative Generator** — Run this curl command via Bash.
   **IMPORTANT: GPT-4o gets `/tmp/sanity-check-problem-only.txt` (problem only, NO approaches).** This forces it to generate fresh alternatives without anchoring.
   ```
   curl -s https://api.openai.com/v1/chat/completions \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer $OPENAI_API_KEY" \
     -d "$(jq -n --arg problem "$(cat /tmp/sanity-check-problem-only.txt)" '{
       model: "gpt-4o",
       temperature: 0.5,
       messages: [
...
2. **Independent Claude — Judge & Evaluator** — Use the Task tool with subagent_type "general-purpose" and this prompt.
   **IMPORTANT: Claude gets `/tmp/sanity-check-problem.txt` (problem WITH approaches).** It evaluates the approaches that were generated.
   ...
   > [paste full text from /tmp/sanity-check-problem.txt]
...
If the user approves, this becomes the input for plan-mode or `/sprint`. The validated approach should be referenced in the plan's introduction so `/critique` reviewers have context on why alternatives were rejected.
